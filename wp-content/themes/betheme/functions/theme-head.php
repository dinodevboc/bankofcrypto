	...
	function mfn_styles()
	{
		...
		foreach ($fonts as $font) {
			if (in_array($font, $google_fonts)) {
				...
				$google_array[$font_slug] = $font_slug . $weight . $subset;
			}
		}

		if ($google_array) {
			$google_array = implode('|', $google_array);
			wp_enqueue_style('mfn-fonts', 'https://fonts.googleapis.com/css?family='. $google_array);
		}
	...
	function mfn_styles_custom()
	{
		...
		if ($layoutID = mfn_layout_ID()) {
			...
			if (get_post_meta($layoutID, 'mfn-post-color-subheader', true)) {
				...
				$layout_styles .= '#Subheader ul.breadcrumbs li, #Subheader ul.breadcrumbs li a {color: '. mfn_hex2rgba(get_post_meta($layoutID, 'mfn-post-color-subheader', true), .6) .';} ';
			}
...
if (! function_exists('mfn_sidebar')) {
	function mfn_sidebar(){
		...
		if ( is_search() ) {
			...
			return mfn_sidebar_one_or_both('right-sidebar', 'mfn-search', false);
